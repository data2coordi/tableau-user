...
# ユーザーアカウントの同期 ##############
# 追加: CSVに存在するがサーバー上にないユーザーを新規作成。
# 削除: サーバー上に存在するがCSVにないユーザーを削除（ただし、保護対象ユーザー SAFEGUARD_USERS は除外）。

# グループおよび所属メンバーの同期 ########
# サーバー上およびCSV上のすべてのグループを処理対象に指定（保護対象グループ SAFEGUARD_GROUPS は除く）。
# グループ作成: CSVにのみ存在するグループをサーバー上に新規作成。
# メンバー追加: CSVの指定通りに、ユーザーを該当グループへ追加。
# メンバー除外: CSVの指定から外れたユーザーを該当グループから脱退（保護対象ユーザーは除く）。
